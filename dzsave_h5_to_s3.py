...
def upload_to_s3(file_or_dir, s3_prefix=""):
    ...
    if object_count > 1000:  # Example limit
        raise Exception(
            f"S3 object limit exceeded under prefix '{s3_prefix}'. Current objects: {object_count}"
        )
    ...
    if recent_puts > 200:
        raise Exception(
            f"Exceeded 200 PUT requests in the last 24 hours. Current count: {recent_puts}"
        )
    ...
    if os.path.isdir(file_or_dir):
        ...
        if len(files_to_upload) > 100:
            raise Exception(
                f"Attempting to upload more than 100 files. Current file count: {len(files_to_upload)}"
            )
...
heatmap_h5_save_path = "/media/hdd3/neo/S3_tmp_dir/heatmaps/test_slide_2_heatmap.h5"
